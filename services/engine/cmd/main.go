...
// main is the entry point for the OCULTAR Refinery Engine CLI and HTTP server.
func main() {
	...
	if *showVersion || *showVersionShort {
		fmt.Printf("OCULTAR Refinery Engine v%s\n", VERSION)
		os.Exit(0)
	}
...
func startServer(eng *engine.Engine, servePort string) {
	...
	http.HandleFunc("/api/reveal", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, t := range payload.Tokens {
			decrypted, err := engine.DecryptToken(eng.Vault, eng.MasterKey, t)
	...
	http.HandleFunc("/api/refine", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.Unmarshal(inputData, &jsonRaw); err == nil {
			...
			if err != nil {
				...
				http.Error(w, "Ocultar Engine: internal engine error", http.StatusInternalServerError)
				return
			}
	...
	http.HandleFunc("/api/refine/file", func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(strings.ToLower(handler.Filename), ".json") {
			...
			if err != nil {
				log.Printf("Engine error JSON: %v", err)
				http.Error(w, "Ocultar Engine: internal engine error", http.StatusInternalServerError)
				return
			}
			...
			for {
				...
				for _, field := range record {
					if strings.TrimSpace(field) == "" {
						...
						if err != nil {
							log.Printf("Engine error CSV: %v", err)
							http.Error(w, "Ocultar Engine: internal engine error", http.StatusInternalServerError)
							return
						}
		...
		for scanner.Scan() {
			...
			refined, err := eng.RefineString(line, actor, nil)
			if err != nil {
				log.Printf("Engine error JSONL: %v", err)
				http.Error(w, "Ocultar Engine: internal engine error", http.StatusInternalServerError)
				return
			}
