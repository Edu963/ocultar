...
// Engine is the OCULTAR core redaction engine.
// The storage backend is fully abstracted behind vault.Provider; the engine
...
// NewEngine constructs an Engine using a vault.Provider as its storage backend.
...
func (e *Engine) RefineBatch(items []interface{}, actor string) ([]interface{}, error) {
	if len(items) == 0 {
		return items, nil
	}
...
func (e *Engine) applyStructuralHeuristics(input string, actor string) (string, error) {
	refined := input
...
func (e *Engine) isFullyTokenised(s string) bool {
...
func (e *Engine) applyRegexShield(input string, re *regexp.Regexp, piiType string, actor string) (string, error) {
...
func (e *Engine) applyReplacement(line, target, piiType string, actor string) (string, error) {
