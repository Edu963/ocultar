...
func NewSyslogServer(eng *engine.Engine, listenAddr, upstreamAddr string) *SyslogServer {
	return &SyslogServer{
		eng:          eng,
...
func (s *SyslogServer) listen(upstream *net.UDPAddr) {
	...
	for {
		...
		msg := string(buf[:n])
		// Send through engine for redaction
		refined, err := s.eng.RefineString(msg, "syslog_proxy", nil)
		if err != nil {
			log.Printf("[SYSLOG-BLOCK] Engine error: %v (dropping message)", err)
