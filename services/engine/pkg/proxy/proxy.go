...
// It holds references to the shared engine and vault for thread-safe operation.
type Handler struct {
	eng       *engine.Engine
...
// The engine is used for Tier 1 + Tier 2 redaction on the request body.
...
//  2. If the body is JSON, runs it through the OCULTAR engine (PII → tokens).
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("[PROXY-BLOCK] Engine error: %v", err)
		if strings.Contains(err.Error(), "trial limit reached") {
			...
			http.Error(w, "ocultar-proxy: internal security engine failure (fail-closed)", http.StatusInternalServerError)
		}
		return
	}
...
// redactBody parses the body as JSON and runs it through the engine's full
// pipeline (Tier 0 → Tier 2). Returns the redacted body bytes, a boolean
// indicating whether any PII was found and replaced, and any critical engine error.
// If the body is not valid JSON it is passed through the engine line-by-line as
...
func streamRefineJSON(dec *json.Decoder, eng *engine.Engine, actor string, out *bytes.Buffer) error {
	t, err := dec.Token()
	if err != nil {
		return err
	}
