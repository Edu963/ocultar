...
func (s *SharePointConnector) Init(config map[string]interface{}, eng *engine.Engine) error {
...
func (s *SharePointConnector) simulateIngestion() {
	...
	for _, doc := range documents {
		// 2. Pass raw data immediately to the engine
		refined, err := s.engine.ProcessInterface(doc, "sharepoint-graph-connector")
		if err != nil {
...
// or as a pkg/internal helper to avoid engine contamination.
