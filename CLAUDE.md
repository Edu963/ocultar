...
| `OCU_LICENSE_KEY` | Base64-encoded Ed25519 license (proxy app only — not read by the refinery server) |
...
| `SLACK_SIGNING_SECRET` | Slack app signing secret for HMAC-SHA256 verification of incoming Slack events (enterprise; required when using the Slack connector). If unset, Sombra rejects all Slack webhook requests with HTTP 500. |
