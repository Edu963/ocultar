#!/usr/bin/env python3
"""Ocultar PII Refinery — Claude MCP Extension (stdio transport).

Two tools:
  refine_text   — redacts PII before sending text to Claude
    ...
    return [
        types.Tool(
            ...
            description=(
                ...
                "text with all PII replaced by deterministic tokens "
                ...
                "properties": {
                    "tokens": {
                        ...
                        "description": (
                            ...
                            "e.g. ['[EMAIL_9c8f7a1b]', '[SSN_3a1b2c4d]']."
                        ),
                    }
                },
                "required": ["tokens"],
...
async def _refine_text(arguments: dict) -> list[types.TextContent]:
    ...
    try:
        ...
        raise RuntimeError(_connection_error("/api/refine"))
    except httpx.TimeoutException:
        raise RuntimeError(
            f"Ocultar /api/refine timed out (15 s). "
...
async def _reveal_tokens(arguments: dict) -> list[types.TextContent]:
    ...
    try:
        ...
        raise RuntimeError(_connection_error("/api/reveal"))
    except httpx.TimeoutException:
        raise RuntimeError(
            f"Ocultar /api/reveal timed out (15 s). "
...
def main_sync() -> None:
    """Entrypoint for the ocultar-claude-mcp console script."""
    asyncio.run(main())
