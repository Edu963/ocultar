...
| `OCU_LICENSE_KEY` | Proxy only | Ed25519 license key read by `apps/proxy` to activate Enterprise proxy features. Not read by the refinery server (`services/refinery/cmd`). | `openssl rand -hex 32` |
| `OCU_JWT_SECRET` | Enterprise (Sombra) | HS256 secret for validating Bearer tokens on Sombra endpoints (`/query`, `/v1/chat/completions`). If unset, all requests are assigned actor `"dev-anonymous"` — insecure outside dev. Generate: `openssl rand -hex 32`. | `openssl rand -hex 32` |
...
| `OCU_PILOT_MODE` | Proxy only | Set to `true` (or `1`) to enforce a hard vault entry cap. Read by `apps/proxy` only — not by the refinery server. | `true` |
