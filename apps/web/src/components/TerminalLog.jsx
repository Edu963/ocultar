...
const TerminalLog = () => {
    ...
    return (
        <div className="relative group w-full max-w-2xl mx-auto mt-12">
            <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500/20 to-sky-500/20 rounded-lg blur opacity-25 group-hover:opacity-40 transition duration-1000"></div>
                ...
                <div ref={containerRef} className="p-6 font-mono text-sm leading-relaxed h-[300px] overflow-y-auto scrollbar-hide">
                    {lines.map((line, i) => (
                        <div key={i} className="mb-1 animate-in fade-in slide-in-from-left-2 duration-300">
                            {line.type === 'input' && <span className="text-emerald-500">{line.text}</span>}
                            ...
                            {line.type === 'action' && <span className="text-emerald-400">{line.text}</span>}
                            {line.type === 'rehydrate' && <span className="text-purple-400">{line.text}</span>}
                        </div>
                    ))}
                    <div className="inline-block w-2 h-5 bg-emerald-500 animate-pulse ml-1 align-middle"></div>
                </div>
            </div>
        </div>
    );
