...
const Dashboard = () => {
  ...
  const handleGeneratePilotReport = async () => {
    ...
    try {
      ...
      setPilotReport({ error: 'Refinery offline or endpoint not yet wired. Run the CLI command directly to generate the report.' });
    } finally {
                  ...
                  <div className="p-5">
                    <div className="flex gap-2 mb-4">
                      <input
                        ...
                        className="flex-1 text-xs font-mono p-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                      />
                      <button
                        onClick={handleGeneratePilotReport}
                        disabled={isGeneratingReport || !pilotDataset}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${isGeneratingReport ? 'bg-slate-200 text-slate-500' : 'bg-blue-600 hover:bg-blue-500 text-white shadow hover:shadow-md active:scale-[0.98]'}`}
                      >
                        <Play className="w-3 h-3" />
                        {isGeneratingReport ? 'Running...' : 'Run'}
                      </button>
                    </div>
