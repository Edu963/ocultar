...
- **Docker**: Docker Engine + Compose plugin (or Docker Desktop on Windows/macOS)
...
2. Boot the `llama.cpp` inference engine (`slm-ner` container) and wait for it to pass its health check.
...
1. **Key Derivation**: On boot, the `OCU_MASTER_KEY` and `OCU_SALT` are passed through **HKDF-SHA256** to derive the 32-byte AES engine key. 
